    ...
    while episode_idx < cfg.dataset.num_episodes_to_record:
        ...
        if teleop_device is not None:
            ...
            if isinstance(teleop_action, dict):
                # Leader arm: motor positions as dict {"motor.pos": value, ...}
                if env.control_mode == "leader":
                    action = torch.tensor(
                        [teleop_action.get(f"{k}.pos", 0.0) for k in env.robot.bus.motors],
                        dtype=torch.float32,
                    )
                # Gamepad/keyboard: EE deltas
                else:
                    action = torch.tensor(
                        [
                ...
                action = torch.tensor(teleop_action, dtype=torch.float32)
            else:
                action = teleop_action if isinstance(teleop_action, torch.Tensor) else torch.tensor(
                    [teleop_action], dtype=torch.float32
                )
        else:
            # No teleoperator: use neutral action (hold position for leader, zeros for gamepad)
            if env.control_mode == "leader":
                ...
                action = torch.zeros(env.action_space.shape[0], dtype=torch.float32)
